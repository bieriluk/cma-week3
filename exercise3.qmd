...
So what does the timelag between measurment points look like:
```{r}
tail(posmo_filter)
...
To try, we fist choose v = 1h, so 30min 
...
Visualization:
```{r}
pivot_longer(Similarity, cols = c("DTW", "EditDist", "Frechet", "LCSS"))

ggplot(Similarity, aes(ID, value, color = ID)) +
  geom_bar() + 
  facet_wrap(name~.)
```
